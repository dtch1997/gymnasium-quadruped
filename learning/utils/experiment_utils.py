...
from learning.utils import wandb_logger

# Fix an issue with launchpad when passing args to python-process
if "lp_dummy_config" not in flags.FLAGS:
    try:
        ...
        config_flags.DEFINE_config_dict("lp_dummy_config", config_dict.ConfigDict())
...
def make_local_resources(program, share_gpu: bool = False):
    """Create default resources for local LP program"""
    import launchpad as lp
    from acme.utils import paths
    from launchpad.nodes.python.local_multi_processing import PythonProcess

    unique_id = paths.get_unique_id()[0]
    if share_gpu:
        cpu_process = PythonProcess(args=dict(acme_id=unique_id))
    else:
        cpu_process = PythonProcess(
            args=dict(acme_id=unique_id), env=dict(CUDA_VISIBLE_DEVICES="")
        )

    gpu_process = PythonProcess(args=dict(acme_id=unique_id), env=dict())

    groups = program.groups
    local_resources = {}

    if "replay" in groups:
        local_resources["replay"] = cpu_process

    if "counter" in groups:
        local_resources["counter"] = cpu_process

    if "actor" in groups:
        local_resources["actor"] = cpu_process

    if "evaluator" in groups:
        local_resources["evaluator"] = cpu_process

    if "learner" in groups:
        local_resources["learner"] = gpu_process

    if "rewarder" in groups:
        local_resources["rewarder"] = gpu_process
