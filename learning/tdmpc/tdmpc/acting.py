...
from learning.tdmpc.tdmpc import planning
        ...
        *,
        ...
        def plan_step(params, obs, state):
            ...
            def critic_fn(params, z, action):
                value = jnp.squeeze(
                    jnp.minimum(*networks.q(params, z, action)), axis=-1
                )
                return value_tx_pair.apply_inv(value)

            def model_fn(params, z, action):
                next_z, reward = networks.next(params, z, action)
                reward = value_tx_pair.apply_inv(reward)
